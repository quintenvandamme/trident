...
void install_wsl_arm64(
  ...
  if (system_kernel.contains('-WSL2')) {
    ...
    install(download_link, file_extension) async {
      ...
      await shell.run(
          '''cp $path_download/wsl2/linux-$kernel_version/arch/arm64/boot/bzImage /mnt/c/Users/$username''');
      await shell.run(
          '''cp $path_download/wsl2/linux-$kernel_version/arch/arm64/boot/vmlinux.bin /mnt/c/Users/$username''');
      wrtite_catalog(
          '[wsl2]\r\nkernel=C:' +
              '$slash' +
              'Users' +
...
void install_wsl_rc_arm64(kernel_version, kernel_type) async {
  String download_link =
      'https://git.kernel.org/torvalds/t/linux-$kernel_version.tar.gz';
  install_wsl_arm64(kernel_version, kernel_type, download_link, '.tar.gz');
}

void install_wsl_mainline_arm64(kernel_version, kernel_type) async {
  String download_link =
